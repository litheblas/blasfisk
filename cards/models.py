...
class MagnetCard(Card):
    person = models.ForeignKey('blasbase.Person', related_name='magnet_cards')  # Kort _måste_ associeras med en person. Låt det vara så så slipper vi "temporära lösningar" och vilsna kort som ingen vet vem de tillhör.
    card_data = models.CharField(max_length=256, verbose_name=_('card data'))


class RFIDCard(Card):
    person = models.ForeignKey('blasbase.Person', related_name='rfid_cards')  # Kort _måste_ associeras med en person. Låt det vara så så slipper vi "temporära lösningar" och vilsna kort som ingen vet vem de tillhör.
